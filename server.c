...
time_t date, dateA;//storing the dates for filtering the files
//Port numbers for Mirror1 and Mirror2

#define port_for_m1 6301
#define port_for_m2 3011
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        // run in mirror 1
        else if (client >= 4 && client <= 6)
        {
            // printf("connected with mirror 1 \n");
            // send(client_socket, "8089", 5, 0);
            printf("Redirecting to mirror\n");
            // mirrorRedirection(client_socket);
            char for_mirror1[1024];
            snprintf(for_mirror1, 1024, "%d\n", port_for_m1);
            send(client_socket, for_mirror1, strlen(for_mirror1), 0);
            close(client_socket);
        }

        // run in mirror2

        else if (client >= 7 && client <= 9)
        {
            printf("Redirecting to mirror2 \n");
            // mirrorRedirection2(client_socket);
            char for_mirror2[1024];
            snprintf(for_mirror2, 1024, "%d\n", port_for_m2);
            send(client_socket, for_mirror2, strlen(for_mirror2), 0);
            close(client_socket);
        }

        else if (client > 9 && client % 3 == 1)
        {
            printf("connected with client \n");
            send(client_socket, "3456", 5, 0);
            int id1 = fork();
